...
// PostAvailabilityJSON handles request for availability and sends JSON response
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	form := forms.New(r.PostForm)
	for _, x := range rooms {
		// compare block map with submitted data
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				if val > 0 {
					...
					// the rest are just placeholders for days without any block
	...
	for name, _ := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			...
			err := m.DB.InsertBlockFormRoom(roomID, t)
			if err != nil {
				log.Println(err)
			}
		}
